...
class TestAPIHabit(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.cron = CrontabSchedule.objects.create(hour=18,
                                                   minute=41,
                                                   )
        self.interval = CrontabSchedule.objects.create(
            minute=0,
            hour=0,
            day_of_month='*/1',
            )
    ...
    def test_create_habit_bad_request_input_two_values(self):
        ...
        self.assertEqual(
            ...
            'related_habit и reward не '
            'могут быть определенны вместе',
            )
    ...
    def test_create_habit_bad_request_nice_habit(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            str(response.data['is_nice_habit'][0]),
    ...
    def test_create_habit_bad_request_related_habbit_is_some_publish(self):
        """Тест проверки вхождения связанной модели
        не с одинаковой публичностью
        """
        habit = Habit.objects.create(owner=self.user,
                                     place='test_place',
                                     time_to_do=self.cron,
        ...
        self.assertEqual(
            str(response.data['related_habit'][0]),
            'Связанная привычка и текущая не могут '
            'иметь разные статусы публичности',
            )
    ...
    def test_habit_retrieve_block_view(self):
        ...
        self.assertEqual(
            str(response.data),
            "{'detail': "
            "ErrorDetail(string='You do not have permission to "
    ...
    def test_habit_update_change_raleted_published(self):
        """Тест изменения публичности у связанной
        привычки при изменении у основной
        """
        ...
        url_habit = reverse('habits:habit_update',
                            kwargs={'pk': habit.pk},
                            )
        ...
        url_ralated = reverse('habits:habit_update',
                              kwargs={'pk': ralated.pk},
                              )
    ...
    def test_habit_update_block_permission(self):
        """Тест блокировки доступа к изменению
        привычки если не является владельцем
        """
    ...
    def test_habit_delete_block_permission(self):
        """Тест блокировки доступа к удалению
        привычки если не является владельцем
        """
        user = get_user_model().objects.create_user('owner',
                                                    'owner@gmail.com',
