...
class HandleInterval:
    ...
    @classmethod
    def _get_or_set_interval(cls, value: int, type_of_time: str) -> CrontabSchedule:
        """Получение или создание интервала
        """
        match value, type_of_time:
            case _, 'days': 
                interval = CrontabSchedule.objects.filter(minute='*', hour='*', day_of_month=value)
                if not interval.exists():
                    interval = CrontabSchedule.objects.create(minute='*', hour='*', day_of_month=value)
                    return interval            
                return interval.get()
            
            case _, 'hours':
                interval = CrontabSchedule.objects.filter(minute='*', hour=value, day_of_month='*')
                if not interval.exists():
                    interval = CrontabSchedule.objects.create(minute='*', hour=value, day_of_month='*')
                    return interval            
                return interval.get()
            
            case _, 'minutes':
                interval = CrontabSchedule.objects.filter(minute=value, hour='*', day_of_month='*')
                if not interval.exists():
                    interval = CrontabSchedule.objects.create(minute=value, hour='*', day_of_month='*')
                    return interval            
                return interval.get()
            
    @classmethod
    def get_interval(cls, value: Union[str, None]) -> CrontabSchedule:
...
class HandleTimeToDo:
    ...
    def _get_or_set_crontab_time(cls, hour: int, minute: int) -> CrontabSchedule:
        ...
        """
        crontab_time = CrontabSchedule.objects.filter(hour=hour, minute=minute, day_of_month='*')
        if not crontab_time.exists():
            crontab_time = CrontabSchedule.objects.create(hour=hour, minute=minute, day_of_month='*')
            return crontab_time
...
class HandleCronScheduleToTask:
    ...
    @classmethod
    def _check_cron_instance(cls, cron_to_do: CrontabSchedule, cron_interval: CrontabSchedule) -> None:
        """Проверка кронтабов
        """        
        if not isinstance(cron_to_do, CrontabSchedule):
            raise TypeError(f'{cron_to_do}, должен быть CrontabSchedule')
        if not isinstance(cron_interval, CrontabSchedule):
            raise TypeError(f'{cron_interval}, должен быть CrontabSchedule')
        
    @classmethod
    def _parse_cron_intervals(cls, cron_to_do: CrontabSchedule, cron_interval: CrontabSchedule) -> tuple:
    ...
    @classmethod
    def _get_or_set_crontab_time(cls, minute: int, hour: int, day_of_month: str) -> CrontabSchedule:
        """Получение или создание Crontab времени
        """
        crontab_time = CrontabSchedule.objects.filter(hour=hour, minute=minute, day_of_month=day_of_month)
        if not crontab_time.exists():
            crontab_time = CrontabSchedule.objects.create(hour=hour, minute=minute, day_of_month=day_of_month)
            return crontab_time
        return crontab_time.get()
    
    @classmethod
    def get_interval_to_task(cls, cron_to_do: CrontabSchedule, cron_interval: CrontabSchedule) -> CrontabSchedule:
        """Перерабатывает два кронтаб объекта в один для задач
        """
        cls._check_cron_instance(cron_to_do, cron_interval)
        minute, hour, day_of_month = cls._parse_cron_intervals(cron_to_do, cron_interval)
        return cls._get_or_set_crontab_time(minute, hour, day_of_month)    
