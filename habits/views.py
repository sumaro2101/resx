...
class HabitCreateAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if owner_related_habit:
            if self.request.user != self.queryset.get(
                    pk=request.data['related_habit']).owner:
                return Response(data={
                    'related_habit':
                        'Связаная привычка может быть только ваша',
...
class HabitRetieveAPIView(generics.RetrieveAPIView):
    """Показание привычки
    """
    queryset = Habit.objects.get_queryset().select_related('owner',
                                                           'time_to_do',
                                                           'related_habit',
                                                           'periodic',
                                                           )
